...
- 제목 끝에 마침표(`.`) 사용 금지
- **Author**: 본인 Git 계정(`user.name` / `user.email`)으로만 커밋 (AI·봇 계정 사용 금지)
...
| `style/<description>`    | UI/스타일 작업 브랜치                     |
| `refactor/<description>` | 리팩토링 브랜치                           |
| `chore/<description>`    | 환경 설정, 리소스 작업 브랜치             |
| `docs/<description>`     | 문서 작업 브랜치                          |
| `test/<description>`     | 테스트 코드 작업 브랜치                   |
| `deps/<description>`     | 의존성 업데이트 브랜치                    |
| `init/<description>`     | 프로젝트 초기 설정 브랜치                 |
| `hotfix/<description>`   | 긴급 버그 수정 브랜치 (커밋 type은 `fix`) |

- 브랜치 `<type>`은 커밋 타입과 동일하게 맞춤 (`hotfix`만 예외)
...
style/portfolio-base-stylesheet
